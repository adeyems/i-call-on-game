...
export function GameView({ roomCode }: { roomCode: string }) {
  ...
  const countdownSecondsLeft =
    countdownEpoch !== null ? Math.max(0, Math.ceil((countdownEpoch - nowEpoch) / 1000)) : null;
  const showLetterModal = !!activeRound && countdownEpoch !== null && nowEpoch < countdownEpoch;
  const isRoundOpen = !!activeRound && countdownEpoch !== null && nowEpoch >= countdownEpoch;
  ...
  const letterPickCountdown =
    letterPickDeadlineEpoch !== null && !activeRound
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          <section className="flex flex-col gap-5">
            ...
            {game.status === "IN_PROGRESS" && !activeRound ? (
              <div className="card-glow p-5">
                <h2 className="mb-2 text-lg font-bold">
                  {isMyTurn ? "Your turn — pick a letter" : "Pick a letter"}
                </h2>
                <LetterGrid
